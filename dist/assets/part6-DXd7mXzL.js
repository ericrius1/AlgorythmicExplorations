import{g as qe,S as Ae,a as He,c as Ve,i as Je,m as Ye}from"./gpu-B6g-pxzC.js";import{B as ve,_ as Xe,H as Qe}from"./hands-Dk_Aa9ga.js";import{O as Ke}from"./camera3d-_003W6Cq.js";import{m as Ze,l as Ce,b as Re,P as Ne,a as et,p as Pe}from"./scrolly-Bq15bFnz.js";const tt=`// Part six's instrument: a 64³ velocity field stirred by hands, kept
...
`,ze=64,pe=ze*ze*ze,R=ze/4,it=10,at=8,rt=Math.tan(.45),Ue=[{name:"ink",mode:0,swirl:1.1,noiseScale:1.7,buoy:0,centerPull:.06,drag:.5,dyeDecay:.45,speedLimit:2.6,drift:.05,flow:1,lifeMin:3,lifeMax:8,colA:[.05,.16,.55],colB:[.1,.55,.95],colC:[.85,.98,1],colorScale:.85,size:.01,stretch:.055,fade:.86,exposure:.8,glow:.32},{name:"embers",mode:1,swirl:.55,noiseScale:2.3,buoy:3.4,centerPull:0,drag:.85,dyeDecay:.7,speedLimit:2.4,drift:.1,flow:1,lifeMin:1.5,lifeMax:4.5,colA:[.45,.05,.01],colB:[1,.45,.08],colC:[1,.92,.55],colorScale:1,size:.008,stretch:.045,fade:.84,exposure:1,glow:.55},{name:"nebula",mode:2,swirl:.6,noiseScale:1.2,buoy:.15,centerPull:.2,drag:.22,dyeDecay:.3,speedLimit:1.7,drift:.03,flow:1,lifeMin:4,lifeMax:10,colA:[.22,.04,.45],colB:[.75,.15,.65],colC:[.45,.95,.9],colorScale:.55,size:.012,stretch:.09,fade:.9,exposure:.75,glow:.28}];class ot{live=new Map;set(a,e,i,b){const c=this.live.get(a);if(c){const s=1/Math.max(b,.001);for(let n=0;n<3;n++){const M=(e[n]-c.pos[n])*s;c.vel[n]+=(Math.max(-5,Math.min(5,M))-c.vel[n])*.35}c.pos=e.slice(),c.shape=i,c.seen=performance.now()}else this.live.set(a,{pos:e.slice(),vel:[0,0,0],shape:i,seen:performance.now()})}drop(a){this.live.delete(a)}pack(a,e){const i=performance.now();let b=0;for(const[c,s]of this.live){if(i-s.seen>250){this.live.delete(c);continue}if(b>=at)break;const n=e+b*12;a[n]=s.pos[0],a[n+1]=s.pos[1],a[n+2]=s.pos[2],a[n+3]=s.shape.radius,a[n+4]=s.vel[0],a[n+5]=s.vel[1],a[n+6]=s.vel[2],a[n+7]=s.shape.push,a[n+8]=s.shape.attract,a[n+9]=s.shape.emit,a[n+10]=s.shape.spin,a[n+11]=1,b++}return b}get count(){return this.live.size}}async function Ee(f,a={}){const e=await qe(),i=new Ae(f,a.hero?.52:.66);if(!e)return He(f);const b=1280;i.canvas.width>b&&(i.canvas.height=Math.round(i.canvas.height*(b/i.canvas.width)),i.canvas.width=b);const c=Ve(i.canvas,e),s=navigator.gpu.getPreferredCanvasFormat(),n=new Ke;n.distance=2.9,n.autoSpin=9e-4,n.attach(i.canvas);const M=e.createShaderModule({code:tt}),p=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:4,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:5,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:6,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:7,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),w=e.createPipelineLayout({bindGroupLayouts:[p]}),m=r=>e.createComputePipeline({layout:w,compute:{module:M,entryPoint:r}}),G=m("advect"),F=m("forces"),E=m("divergence"),I=m("jacobi"),t=m("project"),o=m("particles"),y=e.createBuffer({size:464,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),u=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST,v=[e.createBuffer({size:pe*16,usage:u}),e.createBuffer({size:pe*16,usage:u})],z=[e.createBuffer({size:pe*4,usage:u}),e.createBuffer({size:pe*4,usage:u})],S=e.createBuffer({size:pe*4,usage:u}),O=e.createBuffer({size:2*ve.length*2*16,usage:u});let P=a.count??(a.hero?11e4:24e4),k=null,L=null,$=[],j=0;const H=e.createShaderModule({code:nt}),re=e.createBuffer({size:160,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),ge={color:{srcFactor:"one",dstFactor:"one",operation:"add"},alpha:{srcFactor:"one",dstFactor:"one",operation:"add"}},_=e.createRenderPipeline({layout:"auto",vertex:{module:H,entryPoint:"vsParticle"},fragment:{module:H,entryPoint:"fsParticle",targets:[{format:"rgba16float",blend:ge}]},primitive:{topology:"triangle-list"}}),T=e.createRenderPipeline({layout:"auto",vertex:{module:H,entryPoint:"vsLine"},fragment:{module:H,entryPoint:"fsLine",targets:[{format:"rgba16float",blend:ge}]},primitive:{topology:"line-list"}}),q=e.createRenderPipeline({layout:"auto",vertex:{module:H,entryPoint:"vsFade"},fragment:{module:H,entryPoint:"fsFade",targets:[{format:"rgba16float",blend:{color:{srcFactor:"zero",dstFactor:"constant",operation:"add"},alpha:{srcFactor:"zero",dstFactor:"constant",operation:"add"}}}]},primitive:{topology:"triangle-list"}}),ae=e.createRenderPipeline({layout:"auto",vertex:{module:H,entryPoint:"vsBlit"},fragment:{module:H,entryPoint:"fsBlit",targets:[{format:s}]},primitive:{topology:"triangle-list"}}),te=e.createTexture({size:[i.canvas.width,i.canvas.height],format:"rgba16float",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING}),V=te.createView(),Q=e.createSampler({magFilter:"linear",minFilter:"linear"}),K=e.createBindGroup({layout:ae.getBindGroupLayout(0),entries:[{binding:0,resource:V},{binding:1,resource:Q},{binding:2,resource:{buffer:re}}]}),Y=e.createBindGroup({layout:T.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:re}},{binding:3,resource:{buffer:O}}]});let Fe=null;const Le=()=>{k?.destroy(),L?.destroy(),k=e.createBuffer({size:P*16,usage:u}),L=e.createBuffer({size:P*16,usage:u});const r=new Float32Array(P*4),d=new Float32Array(P*4);for(let h=0;h<P;h++){const C=Math.random()*Math.PI*2,W=Math.acos(Math.random()*2-1),ee=.85*Math.cbrt(Math.random());r[h*4]=Math.sin(W)*Math.cos(C)*ee,r[h*4+1]=Math.sin(W)*Math.sin(C)*ee,r[h*4+2]=Math.cos(W)*ee,r[h*4+3]=Math.random()*6,d[h*4+3]=Math.random()}e.queue.writeBuffer(k,0,r),e.queue.writeBuffer(L,0,d);const N=(h,C)=>e.createBindGroup({layout:p,entries:[{binding:0,resource:{buffer:y}},{binding:1,resource:{buffer:v[h]}},{binding:2,resource:{buffer:v[1-h]}},{binding:3,resource:{buffer:z[C]}},{binding:4,resource:{buffer:z[1-C]}},{binding:5,resource:{buffer:S}},{binding:6,resource:{buffer:k}},{binding:7,resource:{buffer:L}}]});$=[[N(0,0),N(0,1)],[N(1,0),N(1,1)]],Fe=e.createBindGroup({layout:_.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:re}},{binding:1,resource:{buffer:k}},{binding:2,resource:{buffer:L}}]})};Le();let x=Ue[0];const U={colA:[...x.colA],colB:[...x.colB],colC:[...x.colC],exposure:x.exposure,fade:x.fade},ne=new ot;let Me=0,he=!1,me=[1,0,0],xe=[0,0,1],X=[0,1,0];const fe=(r,d,N)=>{const h=n.distance*rt,C=i.canvas.width/i.canvas.height;return[me[0]*r*h*C+xe[0]*d*h-X[0]*N,me[1]*r*h*C+xe[1]*d*h-X[1]*N,me[2]*r*h*C+xe[2]*d*h-X[2]*N]};let ie=1/60;i.canvas.addEventListener("pointerdown",()=>he=!0),i.canvas.addEventListener("pointerup",()=>he=!1),i.canvas.addEventListener("pointercancel",()=>he=!1),i.canvas.addEventListener("pointermove",r=>{if(he)return;const d=i.canvas.getBoundingClientRect(),N=(r.clientX-d.left)/d.width*2-1,h=-((r.clientY-d.top)/d.height*2-1);ne.set("cursor",fe(N,h,0),{radius:.3,push:16,emit:2.2,attract:0,spin:.6},ie),Me=performance.now()}),i.canvas.addEventListener("pointerleave",()=>ne.drop("cursor"));let J=null,de=!1,Z=null;const ue=new Float32Array(2*ve.length*2*4),_e=async()=>{if(de&&J){J.stop(),de=!1,Z&&(Z.textContent="✋ enable hands");return}Z&&(Z.textContent="✋ starting…");try{const{HandTracker:r}=await Xe(async()=>{const{HandTracker:d}=await import("./hands-Dk_Aa9ga.js").then(N=>N.h);return{HandTracker:d}},[]);J??=new r,await J.start(),de=!0,Z&&(Z.textContent="✋ hands on — click to stop")}catch(r){console.error("hand tracking failed",r),Z&&(Z.textContent="✋ camera blocked")}},ye=(r,d)=>[(r*2-1)*1.25,-(d*2-1)*1.25],We=()=>{let r=0;if(ue.fill(0),!(!de||!J))for(const d of J.hands){const N=d.handedness,[h,C]=ye(d.palm[0],d.palm[1]),W=Math.max(-.5,Math.min(.5,d.palm[2]*2));ne.set(`${N}-palm`,fe(h,C,W),{radius:.34,push:13+d.spread*8,emit:1.6,attract:0,spin:.4},ie);const[ee,ke]=ye(d.lm[24],d.lm[25]);if(ne.set(`${N}-index`,fe(ee,ke,W),{radius:.15,push:20,emit:3.5,attract:0,spin:0},ie),d.pinch>.45){const l=(d.lm[12]+d.lm[24])/2,oe=(d.lm[13]+d.lm[25])/2,[se,le]=ye(l,oe),we=(d.pinch-.45)/.55;ne.set(`${N}-pinch`,fe(se,le,W),{radius:.42,push:2,emit:.5,attract:10*we,spin:7*we},ie)}else ne.drop(`${N}-pinch`);Me=performance.now();for(const[l,oe]of ve)for(const se of[l,oe]){const[le,we]=ye(d.lm[se*3],d.lm[se*3+1]),Be=fe(le,we,W);ue[r*4]=Be[0],ue[r*4+1]=Be[1],ue[r*4+2]=Be[2],ue[r*4+3]=.35,r++}}},$e=r=>{if(performance.now()-Me<2500){ne.drop("ghost");return}const d=[.68*Math.sin(r*.63),.68*Math.sin(r*.41+1.3),.55*Math.sin(r*.52+2.6)];ne.set("ghost",d,{radius:.3,push:11,emit:1.2,attract:0,spin:1.2},ie)};let Te=null,Se=null;const Ge=[],je=r=>{x=r,Se?.()};if(a.hero)i.button("✋ hands",()=>void _e()),Z=i.controls.lastElementChild;else{for(const r of Ue)i.button(r.name,()=>je(r)),Ge.push(i.controls.lastElementChild);Se=()=>{Ue.forEach((r,d)=>{Ge[d].textContent=r===x?`● ${r.name}`:r.name})},Se(),i.slider({label:"particles",min:5e4,max:1e6,step:1e4,value:P,log:!0,format:r=>Math.round(r).toLocaleString(),onInput:r=>{P=Math.round(r),Le()}}),i.slider({label:"trails",min:.5,max:.97,step:.01,value:.88,format:r=>`${Math.round(r*100)}%`,onInput:r=>Te=r}),i.button("✋ enable hands",()=>void _e()),Z=i.controls.lastElementChild}i.setInfo(()=>{const r=de?` · ${J?.hands.length??0} hand${J?.hands.length===1?"":"s"}`:"";return`${P.toLocaleString()} particles · ${x.name}${r} — wave, pinch, stir`});const B=new Float32Array(464/4),Oe=new Uint32Array(B.buffer),D=new Float32Array(160/4);let be=0,Ie=performance.now();return{frame(){i.tick();const r=performance.now();ie=Math.min(Math.max((r-Ie)/1e3,1/240),1/30),Ie=r,be+=ie;const d=i.canvas.width/i.canvas.height,{viewProj:N,right:h,up:C}=n.matrices(d);me=h,xe=C,X=[C[1]*h[2]-C[2]*h[1],C[2]*h[0]-C[0]*h[2],C[0]*h[1]-C[1]*h[0]],X=[-X[0],-X[1],-X[2]],We(),$e(be),B[0]=ie,B[1]=be,B[2]=x.swirl,B[3]=x.noiseScale,B[4]=x.buoy,B[5]=x.centerPull,B[6]=x.drag,B[7]=x.dyeDecay,B[8]=x.speedLimit,B[9]=x.drift,B[10]=x.lifeMin,B[11]=x.lifeMax,Oe[12]=P,Oe[14]=x.mode,B[15]=x.flow,B[16]=X[0],B[17]=X[1],B[18]=X[2],B[19]=0,Oe[13]=ne.pack(B,20),e.queue.writeBuffer(y,0,B);const W=1-Math.exp(-ie*3);for(let l=0;l<3;l++)U.colA[l]+=(x.colA[l]-U.colA[l])*W,U.colB[l]+=(x.colB[l]-U.colB[l])*W,U.colC[l]+=(x.colC[l]-U.colC[l])*W;U.exposure+=(x.exposure-U.exposure)*W,U.fade+=((Te??x.fade)-U.fade)*W,D.set(N,0),D.set(h,16),D[19]=x.size,D.set(C,20),D[23]=x.stretch,D.set(U.colA,24),D[27]=x.colorScale,D.set(U.colB,28),D.set(U.colC,32),D[36]=U.exposure,D[37]=d,D[38]=be,D[39]=x.glow*9e3/P,e.queue.writeBuffer(re,0,D),e.queue.writeBuffer(O,0,ue);const ee=e.createCommandEncoder();{const l=ee.beginComputePass(),oe=$[j][0],se=$[j][1];l.setBindGroup(0,oe),l.setPipeline(G),l.dispatchWorkgroups(R,R,R),l.setPipeline(F),l.dispatchWorkgroups(R,R,R),l.setPipeline(E),l.dispatchWorkgroups(R,R,R),l.setPipeline(I);for(let le=0;le<it;le++)l.setBindGroup(0,le%2===0?se:oe),l.dispatchWorkgroups(R,R,R);l.setBindGroup(0,se),l.setPipeline(t),l.dispatchWorkgroups(R,R,R),l.setBindGroup(0,oe),l.setPipeline(o),l.dispatchWorkgroups(Math.ceil(P/256)),l.end()}{const l=ee.beginRenderPass({colorAttachments:[{view:V,loadOp:"load",storeOp:"store"}]});l.setPipeline(q),l.setBlendConstant({r:U.fade,g:U.fade,b:U.fade,a:U.fade}),l.draw(3),l.setPipeline(_),l.setBindGroup(0,Fe),l.draw(6,P),de&&J&&J.hands.length>0&&(l.setPipeline(T),l.setBindGroup(0,Y),l.draw(J.hands.length*ve.length*2)),l.end()}{const l=ee.beginRenderPass({colorAttachments:[{view:c.getCurrentTexture().createView(),clearValue:{r:0,g:0,b:0,a:1},loadOp:"clear",storeOp:"store"}]});l.setPipeline(ae),l.setBindGroup(0,K),l.draw(3),l.end()}e.queue.submit([ee.finish()]);const ke=performance.now();e.queue.onSubmittedWorkDone().then(()=>{window.__gpuMs=performance.now()-ke}),j=1-j},dispose(){J?.stop();for(const r of[...v,...z,S,O,y,re])r.destroy();k?.destroy(),L?.destroy(),te.destroy()}}}const ce=2600;function st(){const f=new Uint8Array(512),a=new Uint8Array(256).map((c,s)=>s);for(let c=255;c>0;c--){const s=Math.floor(Math.random()*(c+1));[a[c],a[s]]=[a[s],a[c]]}f.set(a),f.set(a,256);const e=c=>(c&1?1:-1)*.7,i=c=>c*c*(3-2*c),b=(c,s)=>{const n=Math.floor(c)&255,M=Math.floor(s)&255,p=c-Math.floor(c),w=s-Math.floor(s),m=i(p),G=i(w),F=f[f[n]+M],E=f[f[n+1]+M],I=f[f[n]+M+1],t=f[f[n+1]+M+1],o=(y,u,v)=>y+(u-y)*v;return o(o(e(F)*p+e(F>>1)*w,e(E)*(p-1)+e(E>>1)*w,m),o(e(I)*p+e(I>>1)*(w-1),e(t)*(p-1)+e(t>>1)*(w-1),m),G)};return(c,s,n)=>b(c+n*.18,s+n*.11)+.5*b(c*2.1+13.7-n*.07,s*2.1+5.3)}function lt(f){const a=new Ae(f,.56),e=a.canvas.getContext("2d"),i=a.canvas.width,b=a.canvas.height,c=st();let s=!0,n=2.4,M=0;const p=new Float32Array(ce),w=new Float32Array(ce),m=new Float32Array(ce),G=t=>{p[t]=Math.random(),w[t]=Math.random(),m[t]=2+Math.random()*6};for(let t=0;t<ce;t++)G(t);let F,E;const I=()=>{F.textContent=s?"● curl (swirls)":"curl (swirls)",E.textContent=s?"downhill (sinks)":"● downhill (sinks)"};return a.button("curl",()=>{s=!0,I()}),F=a.controls.lastElementChild,a.button("downhill",()=>{s=!1,I()}),E=a.controls.lastElementChild,I(),a.slider({label:"noise scale",min:1,max:6,step:.1,value:n,onInput:t=>n=t}),a.setInfo(()=>s?`${ce.toLocaleString()} tracers · rotate the gradient 90° and no point gains or loses flow`:`${ce.toLocaleString()} tracers · follow the gradient and everything pools at the peaks`),e.fillStyle="#06070d",e.fillRect(0,0,i,b),{frame(){a.tick(),M+=1/60,e.fillStyle="rgba(6, 7, 13, 0.08)",e.fillRect(0,0,i,b),e.lineWidth=Math.max(1,i/900);const t=.012;for(let o=0;o<ce;o++){const y=p[o],u=w[o],v=(c((y+t)*n,u*n,M)-c((y-t)*n,u*n,M))/(2*t),z=(c(y*n,(u+t)*n,M)-c(y*n,(u-t)*n,M))/(2*t);let S,O;s?(S=z,O=-v):(S=v,O=z);const P=.0035,k=y+S*P,L=u+O*P,$=Math.min(Math.hypot(S,O)*.55,1);e.strokeStyle=`hsla(${205+$*110}, 85%, ${45+$*35}%, 0.55)`,e.beginPath(),e.moveTo(y*i,u*b),e.lineTo(k*i,L*b),e.stroke(),p[o]=k,w[o]=L,m[o]-=1/60;const j=!s&&$<.02;(m[o]<0||j||k<0||k>1||L<0||L>1)&&G(o)}}}}function ct(f){const a=new Ae(f,.62),e=a.canvas.getContext("2d"),i=a.canvas.width,b=a.canvas.height,c=new Qe;let s="idle",n;a.button("📷 start hand tracking",()=>{if(s==="on"){c.stop(),s="idle",n.textContent="📷 start hand tracking";return}s="starting",n.textContent="loading model…",c.start().then(()=>{s="on",n.textContent="stop"}).catch(p=>{console.error("hand tracking failed",p),s="blocked",n.textContent="camera unavailable"})}),n=a.controls.lastElementChild,a.setInfo(()=>{if(s!=="on")return"21 landmarks · 7.7 MB of weights, fetched on demand";const p=c.hands;return p.length===0?"looking for hands…":p.map(w=>`${w.handedness}: pinch ${(w.pinch*100).toFixed(0)}% · spread ${(w.spread*100).toFixed(0)}%`).join(" · ")});const M=[4,8,12,16,20];return{frame(){if(a.tick(),e.fillStyle="#06070d",e.fillRect(0,0,i,b),s!=="on"){e.fillStyle="#5b647f",e.font=`${Math.round(i/38)}px system-ui, sans-serif`,e.textAlign="center",e.fillText(s==="blocked"?"Camera permission was denied — the cursor still works everywhere below.":s==="starting"?"Fetching weights and compiling the WebGPU pipeline…":"Click “start hand tracking”. The video never leaves your machine —",i/2,b/2-10),s==="idle"&&e.fillText("inference runs in this tab, on your GPU.",i/2,b/2+i/30);return}const p=b*(4/3),w=(i-p)/2;e.save(),e.globalAlpha=.5,e.translate(w+p,0),e.scale(-1,1),e.drawImage(c.video,0,0,p,b),e.restore();for(const m of c.hands){const G=v=>[w+m.lm[v*3]*p,m.lm[v*3+1]*b],F=m.handedness==="left"?195:330;e.strokeStyle=`hsla(${F}, 90%, 65%, 0.85)`,e.lineWidth=Math.max(2,i/500),e.beginPath();for(const[v,z]of ve){const[S,O]=G(v),[P,k]=G(z);e.moveTo(S,O),e.lineTo(P,k)}e.stroke();for(let v=0;v<21;v++){const[z,S]=G(v),O=M.includes(v);e.fillStyle=O?`hsl(${F}, 100%, 80%)`:`hsla(${F}, 80%, 60%, 0.9)`,e.beginPath(),e.arc(z,S,O?i/180:i/280,0,Math.PI*2),e.fill()}const[E,I]=G(4),[t,o]=G(8);e.strokeStyle=`hsla(${F}, 100%, 75%, ${.25+m.pinch*.75})`,e.lineWidth=Math.max(2,i/400)*(1+m.pinch*2),e.beginPath(),e.moveTo(E,I),e.lineTo(t,o),e.stroke(),m.pinch>.6&&(e.fillStyle=`hsla(${F}, 100%, 85%, ${m.pinch})`,e.beginPath(),e.arc((E+t)/2,(I+o)/2,i/90*m.pinch,0,Math.PI*2),e.fill());const[y,u]=G(0);e.fillStyle=`hsla(${F}, 70%, 75%, 0.9)`,e.font=`${Math.round(i/50)}px system-ui, sans-serif`,e.textAlign="center",e.fillText(m.handedness,y,u+i/28)}},dispose(){c.stop()}}}const g=22,A=14,dt=16,De=5;function ut(f){const a=new Float32Array(g*A),e=new Float32Array(g*A),i=g*.32,b=A*.55,c=g*.68,s=A*.4;for(let t=0;t<A;t++)for(let o=0;o<g;o++){const y=t*g+o,u=o-i,v=t-b,z=Math.hypot(u,v)+.001,S=Math.exp(-(z*z)/30)*1.4;a[y]+=-v/z*S,e[y]+=u/z*S;const O=o-c,P=t-s,k=Math.hypot(O,P)+.001,L=Math.exp(-(k*k)/14)*1.7;a[y]+=O/k*L,e[y]+=P/k*L}const n=(t,o,y)=>t[Math.min(A-1,Math.max(0,y))*g+Math.min(g-1,Math.max(0,o))],M=new Float32Array(g*A);for(let t=0;t<A;t++)for(let o=0;o<g;o++)M[t*g+o]=(n(a,o+1,t)-n(a,o-1,t)+n(e,o,t+1)-n(e,o,t-1))/2;const p=[];let w=new Float32Array(g*A);p.push(w.slice());for(let t=0;t<dt;t++){for(let o=0;o<De;o++){const y=new Float32Array(g*A);for(let u=0;u<A;u++)for(let v=0;v<g;v++)y[u*g+v]=(n(w,v-1,u)+n(w,v+1,u)+n(w,v,u-1)+n(w,v,u+1)-M[u*g+v])/4;w=y}p.push(w.slice())}const m=p[p.length-1],G=new Float32Array(g*A),F=new Float32Array(g*A);for(let t=0;t<A;t++)for(let o=0;o<g;o++)G[t*g+o]=a[t*g+o]-(n(m,o+1,t)-n(m,o-1,t))/2,F[t*g+o]=e[t*g+o]-(n(m,o,t+1)-n(m,o,t-1))/2;let E=0;for(const t of m)E=Math.max(E,Math.abs(t));let I=0;for(const t of M)I=Math.max(I,Math.abs(t));Ze(f,{screens:4,aspect:.6,steps:[{at:0,text:"A velocity field after stirring: a vortex on the left, and an illegal pile-up on the right where flow pours outward from nothing. Real incompressible fluid forbids that."},{at:.16,text:"Measure the crime: divergence. Red cells create fluid, blue cells swallow it. The vortex barely registers — rotation is legal. The source glows red."},{at:.38,text:"Solve ∇²p = ∇·u for pressure. Here it's Jacobi relaxation: every cell repeatedly averages its neighbours. Scrub slowly — you're watching the pressure field negotiate itself into shape, sweep by sweep."},{at:.68,text:"Subtract the pressure gradient. The outflow collapses, the divergence map goes dark, and what's left is pure swirl — the part of the motion that reads as fluid."}],draw(t,o,y,u){const z=(o-24)/g,S=(y-24-16)/A,O=12,P=12,k=Pe(u,0,.1),L=Pe(u,.16,.34),$=Pe(u,.38,.66),j=Pe(u,.68,.92),H=L*(1-j);if(H>.01)for(let _=0;_<A;_++)for(let T=0;T<g;T++){const q=M[_*g+T]/I;Math.abs(q)<.04||(t.fillStyle=q>0?`rgba(255, 110, 110, ${.45*q*H})`:`rgba(100, 150, 255, ${-.45*q*H})`,t.fillRect(O+T*z,P+_*S,z,S))}if($>0){const _=$*(p.length-1),T=Math.floor(_),q=Math.min(p.length-1,T+1),ae=_-T,te=Math.min(1,$*3)*(1-.55*j);for(let Q=0;Q<A;Q++)for(let K=0;K<g;K++){const Y=Ce(p[T][Q*g+K],p[q][Q*g+K],ae)/(E||1);Math.abs(Y)<.05||(t.fillStyle=Y>0?`rgba(255, 184, 107, ${.4*Math.min(1,Y)*te})`:`rgba(125, 214, 160, ${.4*Math.min(1,-Y)*te})`,t.fillRect(O+K*z,P+Q*S,z,S))}const V=Math.round(_*De);Re(t,`jacobi sweeps: ${V}`,o-14,y-10,{color:Ne.muted,size:11,align:"right",mono:!0,alpha:Math.min(1,$*3)*(1-j)})}const re=2;for(let _=0;_<A;_+=1)for(let T=0;T<g;T+=1){if((T+_)%re!==0)continue;const q=_*g+T,ae=Ce(a[q],G[q],j),te=Ce(e[q],F[q],j),V=Math.hypot(ae,te);if(V<.05)continue;const Q=O+(T+.5)*z,K=P+(_+.5)*S,Y=Math.min(z*1.5,V*z*.9);t.globalAlpha=k*Math.min(1,.35+V*.5),et(t,Q-ae/V*Y*.5,K-te/V*Y*.5,Q+ae/V*Y*.5,K+te/V*Y*.5,Ne.dot,1.3,4.5)}t.globalAlpha=1;const ge=j>0?"u ← u − ∇p":$>0?"∇²p = ∇·u":L>0?"measure divergence":"raw velocity field";Re(t,ge,o-14,20,{color:Ne.muted,size:12,align:"right",mono:!0})}})}Je();const ft={"hero-toy":f=>Ee(f,{hero:!0}),flow:lt,hand:ct,playground:f=>Ee(f,{})};for(const f of document.querySelectorAll("[data-demo]")){const a=f.dataset.demo,e=ft[a];e&&Ye(f,()=>e(f))}const pt={projection:ut};for(const f of document.querySelectorAll("[data-scrolly]"))pt[f.dataset.scrolly]?.(f);
