...
`;async function Xe(i){const e=await fe(),t=new he(i,.62);if(!e)return ue(i);const o=new de(e,t.canvas,Je,3*w);pe(i);const s=[4,2,8],n=[0,4,0];let l=[0,1,2].map(a=>K(s[a],w,n[a]));const x=()=>{l=[0,1,2].map(a=>K(s[a],w,n[a]));for(let a=0;a<3;a++)for(let u=0;u<w;u++)o.data[a*w+u]=l[a][u]?1:0};x();const I=new Ee({pitchDecay:.04,octaves:7,volume:-6}).connect(A()),C=new V(1800,"bandpass").connect(A()),k=new W({noise:{type:"pink"},envelope:{attack:.001,decay:.16,sustain:0},volume:-8}).connect(C),R=new V(8e3,"highpass").connect(A()),T=new W({noise:{type:"white"},envelope:{attack:.001,decay:.045,sustain:0},volume:-14}).connect(R),M=[0,0,0],p=new ye(104,4,(a,u)=>{const _=a%w;l[0][_]&&(I.triggerAttackRelease(55,.12,u),M[0]=1),l[1][_]&&(k.triggerAttackRelease(.16,u),M[1]=1),l[2][_]&&(T.triggerAttackRelease(.05,u),M[2]=1)}),$=()=>p.stop(),O=ge($);t.button("▶ play / stop",()=>{B().then(()=>{p.isRunning?$():p.start()})});const r=["kick hits","snare hits","hat hits"],y=[];for(const a of[0,1,2])y.push(t.slider({label:r[a],min:0,max:16,step:1,value:s[a],format:u=>`E(${u},16)`,onInput:u=>{s[a]=Math.round(u),x()}}));t.slider({label:"snare rotate",min:0,max:15,step:1,value:n[1],onInput:a=>{n[1]=Math.round(a),x()}}),t.slider({label:"tempo",min:70,max:150,step:1,value:p.bpm,format:a=>`${Math.round(a)} bpm`,onInput:a=>p.bpm=a});const f=(a,u,_,v,D)=>{t.button(a,()=>{[s[0],s[1],s[2],n[1]]=[u,_,v,D],y.forEach((j,d)=>j.value=String(s[d])),x(),B().then(()=>{p.isRunning||p.start()})})};return f("four on the floor",4,2,8,4),f("tresillo",3,2,8,4),f("son-ish",5,2,11,4),f("busy",7,5,13,2),t.setInfo(()=>`kick E(${s[0]},16) · snare E(${s[1]},16)+${n[1]} · hat E(${s[2]},16) · evenly spread by Euclid's algorithm`),{frame(){t.tick(),O.pulse();for(let a=0;a<3;a++)M[a]*=.88,o.uniforms[5+a]=M[a];o.uniforms[4]=p.isRunning?p.phase(w):0,o.draw()},dispose(){$()}}}const b=64,q=15,Ke=[0,2,4,5,7,9,11],Se=[{label:"I",degs:[0,2,4]},{label:"IV",degs:[3,5,0]},{label:"V",degs:[4,6,1]},{label:"I",degs:[0,2,4]}],Ue=`
fn scene(uv: vec2f) -> vec3f {
  let t = uf(0u);
...
`;async function Ye(i){const e=await fe(),t=new he(i,.52);if(!e)return ue(i);const o=new de(e,t.canvas,Ue,b+4*q);pe(i);for(let r=0;r<4;r++)for(let y=0;y<q;y++)o.data[b+r*q+y]=Se[r].degs.includes(y%7)?1:0;o.data.fill(-1,0,b);let s=.75,n=.6,l=.8;const x=new Fe({decay:2.2,wet:.25}).connect(A()),I=new J({oscillator:{type:"triangle"},envelope:{attack:.01,decay:.18,sustain:.3,release:.2},volume:-9}).connect(x),C=new Ie(J,{oscillator:{type:"sine"},envelope:{attack:.1,decay:.5,sustain:.4,release:1.2},volume:-19}).connect(x);let k=7,R=0,T=0;const M=r=>60+Ke[r%7]+12*Math.floor(r/7),p=new ye(96,2,(r,y)=>{const f=r%32,a=Math.floor(f/8)%4,u=Se[a];if(f%8===0){for(const D of u.degs)C.triggerAttackRelease(P(M(D)),2.2,y);T=1}if(R=(R+1)%b,Math.random()>l){o.data[R]=-1;return}const _=Math.random();let v;if(_<s*.35?v=k:_<s?v=k+(Math.random()<.5?-1:1):v=k+(Math.random()<.5?-1:1)*(2+Math.floor(Math.random()*3)),f%2===0&&Math.random()<n){let D=v,j=99;for(let d=v-3;d<=v+3;d++){if(d<0||d>=q||!u.degs.includes((d%7+7)%7))continue;const g=Math.abs(d-v);g<j&&(j=g,D=d)}v=D}k=Math.max(0,Math.min(q-1,v)),o.data[R]=k,I.triggerAttackRelease(P(M(k)),.22,y)}),$=()=>{p.stop(),C.releaseAll()},O=ge($);return t.button("▶ play / stop",()=>{B().then(()=>{p.isRunning?$():p.start()})}),t.slider({label:"smoothness (steps vs leaps)",min:0,max:1,step:.01,value:s,onInput:r=>s=r}),t.slider({label:"chord gravity",min:0,max:1,step:.01,value:n,onInput:r=>n=r}),t.slider({label:"note density",min:.2,max:1,step:.01,value:l,onInput:r=>l=r}),t.setInfo(()=>`I–IV–V–I under a random walk · current persona: ${n>.8?"arpeggio-bot":n<.2?"free-roaming":s>.8?"singer":s<.4?"bebop dice":"melody zone"}`),{frame(){t.tick(),O.pulse(),T*=.95,o.uniforms[4]=R,o.uniforms[5]=p.isRunning?Math.floor(p.phase(32)*32/8)%4:0,o.uniforms[6]=T,o.draw()},dispose(){$()}}}const L=64,qe=[{name:"major",steps:[0,2,4,5,7,9,11],flavor:"bright"},{name:"mixolydian",steps:[0,2,4,5,7,9,10],flavor:"sunny slouch"},{name:"dorian",steps:[0,2,3,5,7,9,10],flavor:"hopeful minor"},{name:"aeolian",steps:[0,2,3,5,7,8,10],flavor:"melancholy"},{name:"pentatonic",steps:[0,2,4,7,9],flavor:"no wrong notes"}];function Re(i){let e=i>>>0;return()=>{e|=0,e=e+1831565813|0;let t=Math.imul(e^e>>>15,1|e);return t=t+Math.imul(t^t>>>7,61|t)^t,((t^t>>>14)>>>0)/4294967296}}const Ze=`
fn hash(p: vec2f) -> f32 {
...
`;async function et(i,e={}){const t=e.mode==="hero",o=await fe(),s=new he(i,t?.5:.62);if(!o)return ue(i);const n=new de(o,s.canvas,Ze,L);pe(i,"tap to start the song");const l=new Fe({decay:4,wet:.35}).connect(A()),x=new me("3/8",.3);x.wet.value=.18,x.connect(l);const I=new Ie(J,{oscillator:{type:"triangle"},envelope:{attack:.4,decay:.6,sustain:.5,release:2.5},volume:-17}).connect(l),C=new J({oscillator:{type:"square"},envelope:{attack:.01,decay:.15,sustain:.25,release:.3},volume:-16}).connect(x),k=new J({oscillator:{type:"sine"},envelope:{attack:.02,decay:.25,sustain:.5,release:.4},volume:-9}).connect(A()),R=new Ee({pitchDecay:.04,octaves:7,volume:-7}).connect(A()),T=new V(1900,"bandpass").connect(A()),M=new W({noise:{type:"pink"},envelope:{attack:.001,decay:.15,sustain:0},volume:-11}).connect(T),p=new V(8500,"highpass").connect(A()),$=new W({noise:{type:"white"},envelope:{attack:.001,decay:.04,sustain:0},volume:-17}).connect(p);let O=t?2:0,r=.55,y=Math.floor(Math.random()*1e9),f=Re(y),a=[0,5,3,4],u=[],_=[],v=[],D=0,j=.6,d=7;const g=()=>qe[O],Z=(c,h)=>{const m=g().steps,S=m.length;return h+D+m[(c%S+S)%S]+12*Math.floor(c/S)},ve=c=>[c,c+2,c+4],ee=()=>{f=Re(y),D=Math.floor(f()*12),j=f();const c=g().steps.length,h=[3,5,1][Math.floor(f()*3)]%c,m=[4,3,5][Math.floor(f()*3)]%c,S=f()<.7?4%c:0;a=[0,h,m,S];const N=r;u=K(2+Math.round(N*3),16,0),_=K(f()<.5?2:3,16,4),v=K(4+Math.round(N*9),16,Math.floor(f()*2)),d=c+Math.floor(f()*c)};ee();let ae=0,oe=0;const E=new ye(t?92:100,4,(c,h)=>{const m=c%16,S=Math.floor(c/16)%4,N=a[S],G=r;if(u[m]&&(R.triggerAttackRelease(50,.12,h),ae=1),_[m]&&G>.25&&(M.triggerAttackRelease(.15,h),oe=1),v[m]&&G>.12&&$.triggerAttackRelease(.04,h,.5+.5*Math.random()),m===0){for(const te of ve(N))I.triggerAttackRelease(P(Z(te,60)),4.2,h);k.triggerAttackRelease(P(Z(N,36)),1.8,h)}if(m===8&&G>.4&&k.triggerAttackRelease(P(Z(N,36)),.8,h),m%2===0&&Math.random()<.25+G*.6){const te=Math.random();if(te<.3||(te<.85?d+=Math.random()<.5?-1:1:d+=(Math.random()<.5?-1:1)*(2+Math.floor(Math.random()*2))),m%4===0){const je=ve(N).map(se=>(se%g().steps.length+g().steps.length)%g().steps.length);for(let se=0;se<3&&!je.includes((d%g().steps.length+g().steps.length)%g().steps.length);se++)d+=Math.random()<.5?-1:1}d=Math.max(3,Math.min(2.6*g().steps.length,d)),C.triggerAttackRelease(P(Z(Math.round(d),60)),.18,h,.5+G*.4)}}),ie=()=>{E.stop(),I.releaseAll()},Te=ge(ie),Oe=()=>{E.isRunning?ie():E.start()};s.canvas.addEventListener("pointerdown",()=>{B().then(()=>{E.isRunning||E.start()})}),s.button("▶ play / stop",()=>{B().then(Oe)}),s.button("✨ new song",()=>{y=Math.floor(Math.random()*1e9),ee(),B().then(()=>{E.isRunning||E.start()})});let be;s.button(`mode: ${g().name}`,()=>{O=(O+1)%qe.length,ee(),be.textContent=`mode: ${g().name}`}),be=s.controls.lastElementChild,t||(s.slider({label:"tempo",min:70,max:132,step:1,value:E.bpm,format:c=>`${Math.round(c)} bpm`,onInput:c=>E.bpm=c}),s.slider({label:"energy",min:0,max:1,step:.01,value:r,onInput:c=>{r=c,ee()}})),s.setInfo(()=>{const c=a.map(h=>h+1).join("–");return`${We[D]} ${g().name} (${g().flavor}) · chords on degrees ${c} · seed ${y.toString(36)}`});const le=new Float32Array(L);return{frame(){s.tick(),Te.pulse(),ae*=.9,oe*=.86;const c=ze();for(let h=0;h<L;h++){const m=c[Math.floor(h/L*100)],S=Math.max(0,(Number.isFinite(m)?m+95:0)/60);le[h]+=(Math.min(S,1.6)-le[h])*.25,n.data[h]=le[h]}n.uniforms[4]=ae,n.uniforms[5]=oe,n.uniforms[6]=r,n.uniforms[7]=j,n.draw()},dispose(){ie()}}}Ne();const tt={euclid:i=>Xe(i),walk:i=>Ye(i),jukebox:i=>et(i)};for(const i of document.querySelectorAll("[data-demo]")){const e=tt[i.dataset.demo];e&&Ce(i,()=>e(i))}
