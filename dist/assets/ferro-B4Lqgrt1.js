import{i as ne}from"./siteNav-DaR1fllU.js";import{S as re,g as ie,m as se}from"./demoShell-Btkj803W.js";import{g as ae,c as oe}from"./gpu-DBowy6aD.js";import{G as le,a as V}from"./gridSort2-zMjTnh0y.js";const ce=`// Ferrofluid rendering: the metaball field again — but this time the field's
...
`,fe=256,B=2/V,x={floorY:-.52,topY:.95,wallX:.8},de={gravity:3,stiffness:80,restDensity:2.6,nearStiffness:260,xsph:.16,tension:3.2,magStrength:30,magSoft:.09};function pe(o){const t=B*.5,e=new Float32Array(o*8);let n=0;for(let i=0;n<o&&i<4e3;i++){const l=x.floorY+t*(i+.7),r=x.wallX-t,w=Math.max(Math.floor(r*2/t),1);for(let m=0;m<w&&n<o;m++,n++)e[n*8]=-r+t*(m+.5)+(Math.random()-.5)*t*.4,e[n*8+1]=l+(Math.random()-.5)*t*.4}return e}class ge{count;params;dev;sort;layout;densityPipe;forcePipe;bufs=[null,null];density=null;sortGroups=[null,null];simGroups=[null,null];cur=0;constructor(t,e){this.dev=t,this.count=e,this.sort=new le(t),this.params=t.createBuffer({size:80,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});const n=r=>({type:r});this.layout=t.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:n("uniform")},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:n("storage")},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:n("read-only-storage")},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:n("read-only-storage")},{binding:4,visibility:GPUShaderStage.COMPUTE,buffer:n("storage")}]});const i=t.createShaderModule({code:ue}),l=t.createPipelineLayout({bindGroupLayouts:[this.layout]});this.densityPipe=t.createComputePipeline({layout:l,compute:{module:i,entryPoint:"densityPass"}}),this.forcePipe=t.createComputePipeline({layout:l,compute:{module:i,entryPoint:"forcePass"}}),this.rebuild(e)}rebuild(t){this.count=t;for(const i of this.bufs)i?.destroy();this.density?.destroy();const e=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC;this.bufs=[this.dev.createBuffer({size:t*32,usage:e}),this.dev.createBuffer({size:t*32,usage:e})],this.density=this.dev.createBuffer({size:t*16,usage:GPUBufferUsage.STORAGE}),this.dev.queue.writeBuffer(this.bufs[0],0,pe(t)),this.sortGroups=[this.sort.bindGroup(this.bufs[0],this.bufs[1]),this.sort.bindGroup(this.bufs[1],this.bufs[0])];const n=i=>this.dev.createBindGroup({layout:this.layout,entries:[{binding:0,resource:{buffer:this.params}},{binding:1,resource:{buffer:i}},{binding:2,resource:{buffer:this.sort.starts}},{binding:3,resource:{buffer:this.sort.counts}},{binding:4,resource:{buffer:this.density}}]});this.simGroups=[n(this.bufs[1]),n(this.bufs[0])],this.cur=0}writeParams(t,e,n,i,l){const r=new DataView(new ArrayBuffer(80));r.setUint32(0,this.count,!0),r.setUint32(4,V,!0),r.setFloat32(8,B,!0),r.setFloat32(12,e,!0),r.setFloat32(16,t.gravity,!0),r.setFloat32(20,t.stiffness,!0),r.setFloat32(24,t.restDensity,!0),r.setFloat32(28,t.nearStiffness,!0),r.setFloat32(32,t.xsph,!0),r.setFloat32(36,2e3,!0),r.setFloat32(40,t.tension,!0),r.setFloat32(44,l?t.magStrength:0,!0),r.setFloat32(48,n[0],!0),r.setFloat32(52,n[1],!0),r.setFloat32(56,i[0],!0),r.setFloat32(60,i[1],!0),r.setFloat32(64,t.magSoft,!0),r.setFloat32(68,x.floorY,!0),r.setFloat32(72,x.wallX,!0),r.setFloat32(76,x.topY,!0),this.dev.queue.writeBuffer(this.params,0,r.buffer)}encodeSteps(t,e){this.sort.writeParams(this.count);const n=Math.ceil(this.count/fe);for(let i=0;i<e;i++){this.sort.encode(t,this.sortGroups[this.cur],this.count);const l=t.beginComputePass();l.setBindGroup(0,this.simGroups[this.cur]),l.setPipeline(this.densityPipe),l.dispatchWorkgroups(n),l.setPipeline(this.forcePipe),l.dispatchWorkgroups(n),l.end(),this.cur=1-this.cur}}get current(){return this.bufs[this.cur]}get buffers(){return this.bufs}get currentIndex(){return this.cur}dispose(){this.sort.dispose();for(const t of this.bufs)t?.destroy();this.density?.destroy(),this.params.destroy()}}const _=["final","mesh only","field + contour","particles"],k=65536;async function A(o,t){const e=await ae(),n=new re(o,t.hero?.5:.62);if(!e)return ie(o);const i=oe(n.canvas,e),l=navigator.gpu.getPreferredCanvasFormat(),r=n.canvas.width,w=n.canvas.height,m=r/w,D=1.04,S=[D/m,D],R=Math.floor(r/2),U=Math.floor(w/2),g={...de},F=t.hero?18e3:22e3;let d=t.view??0,G=0,z=0;const u=new ge(e,F),p=e.createShaderModule({code:ce}),M=e.createTexture({size:[R,U],format:"rgba16float",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING}),T=M.createView(),$=e.createSampler({magFilter:"linear",minFilter:"linear"}),v=e.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),C=e.createBuffer({size:k*16,usage:GPUBufferUsage.STORAGE}),y=e.createBuffer({size:4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC}),O=e.createBuffer({size:16,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.INDIRECT}),P=e.createBuffer({size:4,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});let b=!1;const E=e.createRenderPipeline({layout:"auto",vertex:{module:p,entryPoint:"vsSplat"},fragment:{module:p,entryPoint:"fsSplat",targets:[{format:"rgba16float",blend:{color:{srcFactor:"one",dstFactor:"one",operation:"add"},alpha:{srcFactor:"one",dstFactor:"one",operation:"add"}}}]},primitive:{topology:"triangle-list"}}),L=e.createRenderPipeline({layout:"auto",vertex:{module:p,entryPoint:"vsFull"},fragment:{module:p,entryPoint:"fsFill",targets:[{format:l}]},primitive:{topology:"triangle-list"}}),I=e.createRenderPipeline({layout:"auto",vertex:{module:p,entryPoint:"vsSegs"},fragment:{module:p,entryPoint:"fsSegs",targets:[{format:l,blend:{color:{srcFactor:"one",dstFactor:"one-minus-src-alpha",operation:"add"},alpha:{srcFactor:"one",dstFactor:"one-minus-src-alpha",operation:"add"}}}]},primitive:{topology:"triangle-list"}}),Y=e.createRenderPipeline({layout:"auto",vertex:{module:p,entryPoint:"vsDots"},fragment:{module:p,entryPoint:"fsDots",targets:[{format:l,blend:{color:{srcFactor:"one",dstFactor:"one-minus-src-alpha",operation:"add"},alpha:{srcFactor:"one",dstFactor:"one-minus-src-alpha",operation:"add"}}}]},primitive:{topology:"triangle-list"}}),N=e.createComputePipeline({layout:"auto",compute:{module:p,entryPoint:"msCells"}}),W=e.createComputePipeline({layout:"auto",compute:{module:p,entryPoint:"msIndirect"}});let X=[null,null],K=[null,null];const H=()=>{X=u.buffers.map(s=>e.createBindGroup({layout:E.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v}},{binding:1,resource:{buffer:s}}]})),K=u.buffers.map(s=>e.createBindGroup({layout:Y.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v}},{binding:1,resource:{buffer:s}}]}))};H();const j=e.createBindGroup({layout:L.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v}},{binding:2,resource:T},{binding:3,resource:$}]}),J=e.createBindGroup({layout:N.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v}},{binding:2,resource:T},{binding:4,resource:{buffer:C}},{binding:5,resource:{buffer:y}}]}),Q=e.createBindGroup({layout:W.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v}},{binding:5,resource:{buffer:y}},{binding:6,resource:{buffer:O}}]}),Z=e.createBindGroup({layout:I.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v}},{binding:7,resource:{buffer:C}}]});let q=null;n.canvas.addEventListener("pointermove",s=>{const a=n.canvas.getBoundingClientRect(),f=(s.clientX-a.left)/a.width*2-1,c=-((s.clientY-a.top)/a.height*2-1);q=[f/S[0],c/S[1]]}),n.canvas.addEventListener("pointerleave",()=>q=null);const ee=()=>[.42*Math.sin(G*.55),-.02+.3*Math.sin(G*.83+1.3)];if(t.full){n.slider({label:"surface tension",min:0,max:8,step:.1,value:g.tension,onInput:a=>g.tension=a}),n.slider({label:"magnet strength",min:0,max:90,step:1,value:g.magStrength,onInput:a=>g.magStrength=a}),n.slider({label:"magnet reach",min:.05,max:.2,step:.005,value:g.magSoft,onInput:a=>g.magSoft=a}),n.button(`view: ${_[d]}`,function(){d=(d+1)%_.length});const s=n.controls.querySelectorAll("button")[0];s?.addEventListener("click",()=>s.textContent=`view: ${_[d]}`),n.button("re-pour",()=>{u.rebuild(F),H()})}n.setInfo(()=>t.hero?`${F.toLocaleString()} particles · surface remeshed every frame · your cursor is the magnet`:`${F.toLocaleString()} particles · ${z.toLocaleString()} segments by marching squares · cursor = magnet`);const te=(s,a)=>{const f=new Float32Array([S[0],S[1],R,U,B*5.4,1.05,.85,G,s[0],s[1],a?1:0,d,B*.62,k,0,0]);e.queue.writeBuffer(v,0,f)};{u.writeParams(g,.0016,[0,5],[0,1],!1);for(let s=0;s<3;s++){const a=e.createCommandEncoder();u.encodeSteps(a,200),e.queue.submit([a.finish()])}}return{frame(){n.tick(),G+=1/60;const s=!0,a=q??ee();u.writeParams(g,.0016,a,[0,1],s),te(a,s);const f=e.createCommandEncoder();u.encodeSteps(f,5);let c=f.beginRenderPass({colorAttachments:[{view:T,clearValue:{r:0,g:0,b:0,a:0},loadOp:"clear",storeOp:"store"}]});c.setPipeline(E),c.setBindGroup(0,X[u.currentIndex]),c.draw(6,u.count),c.end(),f.clearBuffer(y);const h=f.beginComputePass();h.setPipeline(N),h.setBindGroup(0,J),h.dispatchWorkgroups(Math.ceil(R/8),Math.ceil(U/8)),h.setPipeline(W),h.setBindGroup(0,Q),h.dispatchWorkgroups(1),h.end(),b||f.copyBufferToBuffer(y,0,P,0,4),c=f.beginRenderPass({colorAttachments:[{view:i.getCurrentTexture().createView(),clearValue:{r:.016,g:.018,b:.03,a:1},loadOp:"clear",storeOp:"store"}]}),(d===0||d===2)&&(c.setPipeline(L),c.setBindGroup(0,j),c.draw(3)),(d===0||d===1)&&(c.setPipeline(I),c.setBindGroup(0,Z),c.drawIndirect(O,0)),d===3&&(c.setPipeline(Y),c.setBindGroup(0,K[u.currentIndex]),c.draw(6,u.count)),c.end(),e.queue.submit([f.finish()]),b||(b=!0,P.mapAsync(GPUMapMode.READ).then(()=>{z=Math.min(new Uint32Array(P.getMappedRange())[0],k),P.unmap(),b=!1}).catch(()=>b=!1))},dispose(){u.dispose(),M.destroy(),v.destroy(),C.destroy(),y.destroy(),O.destroy(),P.destroy()}}}ne();const ve={"hero-ferro":o=>A(o,{hero:!0}),mesh:o=>A(o,{view:1}),ferro:o=>A(o,{full:!0})};for(const o of document.querySelectorAll("[data-demo]")){const t=o.dataset.demo,e=ve[t];e&&se(o,()=>e(o))}
