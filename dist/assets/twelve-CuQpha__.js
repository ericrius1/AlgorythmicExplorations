import{i as J}from"./siteNav-RZaw0VT9.js";import{S as F,g as C,m as X}from"./demoShell-Btkj803W.js";import{g as P}from"./gpu-DBowy6aD.js";import{S as R}from"./shaderCanvas-B9soO7i5.js";import{s as j,S as z,m as L,f as B,u as b,N as V,G as _,O as A}from"./audio-C7YF5wtJ.js";import{P as W}from"./PolySynth-D_92O0qp.js";const T=1200*Math.log2(3/2),k=13,K=130.81,Q=`
...
`;async function oe(n){const w=await P(),r=new F(n,.52);if(!w)return C(n);const u=new R(w,r.canvas,te,16);j(n);const v=o=>{const i=new _(0).connect(L()),c=o.freqs.map(M=>{const p=new A(M,"sine").connect(i);return p.partials=Array.from({length:Z},(t,a)=>1/(a+1)**1.4),p});return{gain:i,oscs:c}},l=v(H),s=v(N);let f=0,m=0,h=0;const d=o=>{f=o==="just"?1:0,m=o==="et"?1:0;for(const i of[l,s])for(const c of i.oscs)c.state!=="started"&&c.start();l.gain.gain.rampTo(f*.16,.12),s.gain.gain.rampTo(m*.16,.12)},S=B(()=>{clearInterval(h),d("off")});r.button("▶ pure 4:5:6",()=>{b().then(()=>{clearInterval(h),d(f?"off":"just")})}),r.button("▶ equal-tempered",()=>{b().then(()=>{clearInterval(h),d(m?"off":"et")})}),r.button("alternate A/B",()=>{b().then(()=>{clearInterval(h);let o=!0;d("just"),h=window.setInterval(()=>{o=!o,d(o?"just":"et")},1600)})}),r.button("silence",()=>{clearInterval(h),d("off")});const q=(o,i)=>{const[c,M,p,t]=ee[i];return Math.abs(o.freqs[c]*p-o.freqs[M]*t)};r.setInfo(()=>{const o=q(N,0).toFixed(1),i=q(N,1).toFixed(1);return`tempered third beats ${o}×/s · tempered fifth only ${i}×/s · pure side: 0`});const y=new Float32Array(6);let e=performance.now();return{frame(){r.tick(),S.pulse();const o=performance.now(),i=(o-e)/1e3;e=o;for(let c=0;c<2;c++){const M=c===0?H:N;let p=0;for(let t=0;t<3;t++){const a=q(M,t);y[c*3+t]+=2*Math.PI*a*i;const g=a<.05?1:.5+.5*Math.cos(y[c*3+t]);u.data[c*8+t]=g,u.data[c*8+3+t]=a,p+=Math.min(a/12,1)}u.data[c*8+6]=p/3}u.uniforms[4]=f,u.uniforms[5]=m,u.draw()},dispose(){clearInterval(h),d("off")}}}const I=5,ne=31,x=ne-I+1,E=1200*Math.log2(3/2),U=1200*Math.log2(5/4),D=196,se=`
fn scene(uv: vec2f) -> vec3f {
  let t = uf(0u);
  let sel = uf(4u);
  let flash = uf(5u);
...
`;async function ae(n){const w=await P(),r=new F(n,.52);if(!w)return C(n);const u=new R(w,r.canvas,se,x*2);j(n,"click a column");const v=(e,o)=>{const i=1200/e;return Math.abs(Math.round(o/i)*i-o)};for(let e=0;e<x;e++)u.data[e]=v(I+e,E),u.data[x+e]=v(I+e,U);let l=12,s=0;const f=new _(0).connect(L()),m=new A(D,"sine").connect(f),h=new A(D*1.5,"sine").connect(f);for(const e of[m,h])e.partials=[1,.5,.33,.25,.2];let d=0;const S=()=>{clearTimeout(d),f.gain.rampTo(0,.1)},q=B(S),y=e=>{const o=1200/e,i=Math.round(E/o)*o;m.state!=="started"&&(m.start(),h.start()),clearTimeout(d),h.frequency.value=D*1.5,f.gain.rampTo(.2,.08),d=window.setTimeout(()=>{h.frequency.rampTo(D*Math.pow(2,i/1200),.03),d=window.setTimeout(()=>f.gain.rampTo(0,.3),1400)},1400)};r.canvas.addEventListener("pointerdown",()=>{const e=Math.floor((u.pointer.x-.05)/.9*x);e<0||e>=x||(l=I+e,s=1,b().then(()=>y(l)))});for(const e of[12,19,31])r.button(`${e}-TET`,()=>{l=e,s=1,b().then(()=>y(e))});return r.setInfo(()=>{const e=v(l,E),o=v(l,U);return`${l} equal steps · fifth off by ${e.toFixed(2)}¢ · major third off by ${o.toFixed(2)}¢ · click = pure fifth, then ${l}-TET's`}),{frame(){r.tick(),q.pulse(),s*=.95,u.uniforms[4]=l,u.uniforms[5]=s,u.draw()},dispose(){S()}}}J();const ce={spiral:n=>Y(n),comma:n=>oe(n),whytwelve:n=>ae(n)};for(const n of document.querySelectorAll("[data-demo]")){const w=ce[n.dataset.demo];w&&X(n,()=>w(n))}
