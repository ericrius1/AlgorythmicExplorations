import{g as j,S as D,a as F,i as L,m as G}from"./gpu-fVucxd0a.js";import{S as C}from"./shaderCanvas-B9soO7i5.js";import{s as T,G as q,m as B,O as M,f as z,u as P,c as H,t as R,p as O}from"./audio-C03jcV_H.js";const x=220,U=`
...
`;async function V(l){const w=await j(),i=new D(l,.5);if(!w)return F(l);const u=new C(w,i.canvas,U,4);T(l);let n=3,h=!1;const s=new q(0).connect(B()),d=new M(x,"sine").connect(s),m=new M(x+n,"sine").connect(s),p=o=>{h=o,o?(d.state!=="started"&&(d.start(),m.start()),s.gain.rampTo(.3,.15)):s.gain.rampTo(0,.15)},$=z(()=>{p(!1),b()});let g;const b=()=>{g.textContent=h?"■ stop":"▶ play both"};i.button("▶ play both",()=>{P().then(()=>{p(!h),b()})}),g=i.controls.lastElementChild,i.slider({label:"detune",min:-40,max:40,step:.1,value:n,format:o=>`${o>=0?"+":""}${o.toFixed(1)} Hz`,onInput:o=>{n=o,m.frequency.rampTo(x+n,.03)}}),i.setInfo(()=>{const o=Math.abs(n),a=Math.abs(H((x+n)/x)).toFixed(0),e=o<.2?"unison — one steady tone":o<8?`beating ${o.toFixed(1)}× per second`:o<25?"too fast to count — roughness":"splitting into two notes";return`220 Hz + ${(x+n).toFixed(1)} Hz (${a}¢ apart) · ${e}`});let E=0,y=performance.now();return{frame(){i.tick(),$.pulse();const o=performance.now();E+=2*Math.PI*n*((o-y)/1e3),y=o,u.uniforms[4]=n,u.uniforms[5]=E%(2*Math.PI*1e3),u.draw()},dispose(){p(!1)}}}const A=480,I=220,X=8,v=[{r:1,label:"1:1",name:"unison"},{r:9/8,label:"9:8",name:"major second"},{r:6/5,label:"6:5",name:"minor third"},{r:5/4,label:"5:4",name:"major third"},{r:4/3,label:"4:3",name:"perfect fourth"},{r:3/2,label:"3:2",name:"perfect fifth"},{r:5/3,label:"5:3",name:"major sixth"},{r:2,label:"2:1",name:"octave"}],J=`
fn scene(uv: vec2f) -> vec3f {
  let t = uf(0u);
  let aspect = uf(1u);
  var col = vec3f(0.0);
...
`;async function Q(l,w={}){const i=w.mode==="hero",u=await j(),n=new D(l,i?.42:.62);if(!u)return F(l);const h=new C(u,n.canvas,J,A*2);T(l,i?"tap to hear the interval":"tap for sound");let s=3/2,d=s,m=!1;const p=new q(0).connect(B()),$=new M(I,"sine").connect(p),g=new M(I*s,"sine").connect(p),b=a=>{m=a,a?($.state!=="started"&&($.start(),g.start()),p.gain.rampTo(.28,.2)):p.gain.rampTo(0,.2)},E=z(()=>b(!1)),y=a=>{s=a,g.frequency.rampTo(I*a,.05)};if(i){n.canvas.addEventListener("pointerdown",()=>{P().then(()=>b(!m))});let a=5;window.setInterval(()=>{a=(a+1)%v.length,y(v[a].r)},7e3)}else{n.button("▶ play interval",()=>{P().then(()=>b(!m))});const a=n.controls.lastElementChild;window.setInterval(()=>a.textContent=m?"■ stop":"▶ play interval",300);let e;e=n.slider({label:"frequency ratio",min:1,max:2.05,step:5e-4,value:s,format:t=>t.toFixed(4),onInput:t=>y(t)});for(const t of v)n.button(t.label,()=>{y(t.r),e.value=String(t.r),e.dispatchEvent(new Event("input"))})}n.setInfo(()=>{const a=v.find(t=>Math.abs(H(s/t.r))<4),e=a?`${a.label} — ${a.name}, the figure closes`:`${s.toFixed(4)} — irrational territory, the figure precesses`;return`${I} Hz × ${(I*s).toFixed(1)} Hz · ${e}`});let o=0;return{frame(){n.tick(),E.pulse(),o+=.0035,d+=(s-d)*.08;for(let a=0;a<A;a++){const e=a/(A-1)*X;h.data[a*2]=Math.sin(2*Math.PI*e+o)*.96,h.data[a*2+1]=Math.sin(2*Math.PI*d*e)*.96}h.draw()},dispose(){b(!1)}}}const f=165,r=8,c=256,S=2.05,Y=4.4,_=`
fn scene(uv: vec2f) -> vec3f {
  let t = uf(0u);
  let marker = uf(4u);
  let rough = uf(5u);
...
`;async function K(l){const w=await j(),i=new D(l,.6);if(!w)return F(l);const u=new C(w,i.canvas,_,c+2*r+8);T(l);let n=1e-9;const h=new Float32Array(c);for(let e=0;e<c;e++){const t=1+e/(c-1)*(S-1);h[e]=R(f,f*t,r),n=Math.max(n,h[e])}for(let e=0;e<c;e++)u.data[e]=h[e]/n*.92+.04;for(let e=0;e<8;e++)u.data[c+2*r+e]=(v[e].r-1)/(S-1);let s=3/2,d=!1;const m=new q(0).connect(B()),p=e=>{const t=new M(e,"sine").connect(m);return t.partials=Array.from({length:r},(k,N)=>1/(N+1)),t},$=p(f),g=p(f*s),b=e=>{d=e,e?($.state!=="started"&&($.start(),g.start()),m.gain.rampTo(.22,.15)):m.gain.rampTo(0,.15)},E=z(()=>b(!1));i.button("▶ play",()=>{P().then(()=>b(!d))});const y=i.controls.lastElementChild;window.setInterval(()=>y.textContent=d?"■ stop":"▶ play",300);const o=i.slider({label:"interval (ratio)",min:1,max:S,step:5e-4,value:s,format:e=>e.toFixed(4),onInput:e=>{s=e,g.frequency.rampTo(f*s,.04)}});for(const e of[v[0],v[2],v[3],v[4],v[5],v[7]])i.button(e.label,()=>{o.value=String(e.r),o.dispatchEvent(new Event("input"))});i.button("tritone-ish",()=>{o.value=String(Math.SQRT2),o.dispatchEvent(new Event("input"))}),i.setInfo(()=>{const e=R(f,f*s,r)/n,t=v.find(k=>Math.abs(H(s/k.r))<5);return`${t?`${t.label} ${t.name}`:`ratio ${s.toFixed(3)}`} · roughness ${(e*100).toFixed(0)}%`});const a=e=>Math.log2(e/f)/Y+.02;return{frame(){i.tick(),E.pulse();for(let t=0;t<r;t++)u.data[c+t]=a(f*(t+1)),u.data[c+r+t]=a(f*s*(t+1));u.uniforms[4]=(s-1)/(S-1);let e=0;for(let t=1;t<=r;t++)for(let k=1;k<=r;k++)e+=O(f*t,1/t,f*s*k,1/k);u.uniforms[5]=d?Math.min(e/n*.9,1):0,u.draw()},dispose(){b(!1)}}}L();const W={beats:l=>V(l),lissajous:l=>Q(l),comb:l=>K(l)};for(const l of document.querySelectorAll("[data-demo]")){const w=W[l.dataset.demo];w&&G(l,()=>w(l))}
